<div x-data="{ isOpen: false }">
    ...
    <div id="chat-wrapper" class="chat-wrapper chat-wrapper--ready shadow border-top border-left border-right"
        x-bind:class="isOpen ? 'chat--visible' : ''" data-username="{{ auth()->user()->username }}"
        data-avatar="{{ auth()->user()->avatar }}">

        <div class="chat-title-bar">Chat
            <span x-on:click="isOpen = false" class="chat-title-bar-close"><i class="fas fa-times-circle"></i></span>
        </div>
        ...
        <form wire:submit="send" id="chatForm" class="chat-form border-top">
            <input wire:model="message" type="text" class="chat-field" id="chatField" placeholder="Type a message…"
                autocomplete="off">
        </form>
    </div>
</div>
